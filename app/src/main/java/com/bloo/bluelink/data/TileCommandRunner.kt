...
object TileCommandRunner {
     ...
     *
     * Order of operations:
     * 1. Look up the car's last-known [VehicleSnapshot] from [SnapshotStore]. If
     ...
     *    no vehicle to convert and no repo worth building.
     * 2. Build a fresh [VehicleRepository] for the car's brand, using whatever
     *    session/credentials are currently stored -- one repo per call, not
     *    cached, so it always reflects the latest signed-in session.
     * 3. Acquire [BlueLinkGate.statusMutex] via [withLock] *before* dispatching
     *    the actual command. This is the same app-wide mutex that
     ...
     * 4. Inside the lock, dispatch on [cmd]:
     ...
     * 5. On success: logs the resulting message, then -- *outside* any special
     *    handling, just a best-effort [runCatching] -- writes an optimistic
     *    snapshot update (see [optimistic]) so the tile/widgets flip to the new
     *    state immediately rather than waiting for the next real status refresh
     *    to confirm it.
     ...
     *    [Result].
     *
     * The mutex is released automatically when [withLock]'s block (the
     * `runCatching` above) returns or throws, i.e. before the success/failure
     * post-processing in step 5/6 runs -- so logging and the optimistic
     * snapshot write happen outside the critical section.
     */
