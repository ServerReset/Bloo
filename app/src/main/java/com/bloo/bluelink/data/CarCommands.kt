 ...
 *
 * The rule lives here because [CarCommandRunner.execute] is in `:shared` and cannot
 ...
 */
suspend fun runCarCommand(context: Context, command: CarCommand): CarCommandResult {
    ...
    val resolved = runCatching {
        SnapshotStore(context).current().vehicles.firstOrNull { it.vin == command.vin }
            ?.let { CarCommandRunner.resolveToggle(it, command.action) }
    }.getOrNull() ?: command.action

    val result = CarCommandRunner.execute(context, command)
    ...
    if (result.ok && resolved == CarAction.CLIMATE_OFF) {
